...
class UNet_Binary(nn.Module):
    def __init__(
        ...
        for i in reversed(range(2, len(channels))):
            self.up_layers.append(
                nn.Sequential(
                    DoubleConv(channels[i]*2, channels[i - 1]),
                    nn.ConvTranspose2d(channels[i - 1], channels[i - 1], 2, stride=2),
                )
            )

        self.up_layers.append(DoubleConv(channels[1]*2, channels[0]))

        self.final_conv = nn.Sequential(
            nn.Conv2d(channels[0], out_channels, kernel_size=1, stride=1, padding=0),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        for i in range(len(self.up_layers)-1):
            ...
            skip_connection = TF.resize(skip[i], size=x.shape[2:])
            x = torch.cat((skip_connection, x), dim=1)
        x = self.up_layers[-1](x)
        A = self.final_conv(x)
        return TF.resize(A, shape)
